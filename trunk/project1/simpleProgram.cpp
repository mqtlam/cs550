...
int** discretizeData(float** data, int nRows, int nCols)
{
	...
	for (int row = 0; row < nRows; row++)
	{
		for (int col = 0; col < nCols; col++)
		{
			...
			{
				discretized[row][col] = int( (temp-smallestTempValue)/(largestTempValue-smallestTempValue) * (NUM_BUCKETS - 1)  + 0.5 );
			}
		}
	}
...
void init(int** discretizedData)
{
	...
	for (int row = 0; row < g_nRows; row++)
	{
		for (int col = 0; col < g_nCols; col++)
		{
			...
			vertices[index+1] = vec2((col+1)*horzUnit-1, row*vertUnit-1);
			vertices[index+2] = vec2((col+1)*horzUnit-1, (row+1)*vertUnit-1);
			// second triangle
			vertices[index+3] = vec2(col*horzUnit-1, row*vertUnit-1);
			vertices[index+4] = vec2(col*horzUnit-1, (row+1)*vertUnit-1);
			vertices[index+5] = vec2((col+1)*horzUnit-1, (row+1)*vertUnit-1);
			...
			{
				// color with value
				float hsv[3] = {240*(1.0-1.0*bucket/NUM_BUCKETS), 1, 1};
				HSVtoRGB(hsv,rgb);
			}
    ...
    glBufferData(GL_ARRAY_BUFFER, g_NumPoints*sizeof(vec2), vertices, GL_STATIC_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, g_NumPoints*sizeof(vec3), colors, GL_STATIC_DRAW);
