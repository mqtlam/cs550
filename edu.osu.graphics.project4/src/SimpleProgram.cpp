...
// number of shades for toon shader
int numShades = 4;
...
public:
	...
	void init() {
		...
		GLuint vPosition = glGetAttribLocation(currentShaderProgram, "vPosition");
		glEnableVertexAttribArray(vPosition);
		glVertexAttribPointer(vPosition, 4, GL_FLOAT, GL_FALSE, 0,
				BUFFER_OFFSET(0));

		GLuint vNormal = glGetAttribLocation(currentShaderProgram, "vNormal");
...
void display(void) {
	...
	view = glGetUniformLocation(currentShaderProgram, "View");
	...
	glUniform4fv( glGetUniformLocation(currentShaderProgram, "LightPosition"), 1,
			light_position);
...
void init() {
	...
	phongShaderProgram = InitShader("shaders/vshader_phong.glsl",
			"shaders/fshader_phong.glsl");
	toonShaderProgram = InitShader("shaders/vshader_toon.glsl",
			"shaders/fshader_toon.glsl");
	mainShaderProgram = InitShader("shaders/vshader.glsl",
			"shaders/fshader.glsl");
	currentShaderProgram = mainShaderProgram;
	...
	for (int i = 0; i < 3; i++)
	{
		if (i == 0) {
			...
			currentShaderProgram = toonShaderProgram;
			glUseProgram(currentShaderProgram);
		}
		...
		glUniform4fv( glGetUniformLocation(currentShaderProgram, "AmbientProduct"), 1,
				ambient_product);
		glUniform4fv( glGetUniformLocation(currentShaderProgram, "DiffuseProduct"), 1,
				diffuse_product);
		glUniform4fv( glGetUniformLocation(currentShaderProgram, "SpecularProduct"), 1,
				specular_product);

		glUniform4fv( glGetUniformLocation(currentShaderProgram, "LightPosition"), 1,
				light_position);

		glUniform1f( glGetUniformLocation(currentShaderProgram, "Shininess"),
				material_shininess);

		//Set up selection colors and a flag
		SelectColorRLoc = glGetUniformLocation(currentShaderProgram,
				"selectionColorR");
		SelectColorGLoc = glGetUniformLocation(currentShaderProgram,
				"selectionColorG");
		SelectColorBLoc = glGetUniformLocation(currentShaderProgram,
				"selectionColorB");
		SelectColorALoc = glGetUniformLocation(currentShaderProgram,
				"selectionColorA");
		...
		SelectFlagLoc = glGetUniformLocation(currentShaderProgram, "flag");
		glUniform1i(SelectFlagLoc, flag);

		manipulatorFlagLoc = glGetUniformLocation(currentShaderProgram,
				"manipulatorFlag");
		glUniform1i(manipulatorFlagLoc, 0);

		NumShadesLoc = glGetUniformLocation(currentShaderProgram,
		"numToonShades");
		glUniform1i(NumShadesLoc, numShades);

		// Retrieve transformation uniform variable locations
		//model_view = glGetUniformLocation(program, "ModelView");
		model = glGetUniformLocation(currentShaderProgram, "Model");
		view = glGetUniformLocation(currentShaderProgram, "View");
		projection = glGetUniformLocation(currentShaderProgram, "Projection");
...
void motion(int x, int y) {
	...
	if (currentModelTransformMode == OBJECT_TRANSLATE) {
		...
		//r = sqrt(pow(fx-ax, 2)+pow(fy-ay, 2)+pow(fz-az, 2));
...
void shaderMenuCallback(int id) {
	if (id == 1) {
		currentShaderProgram = mainShaderProgram;
		glUseProgram(currentShaderProgram);
		glutPostRedisplay();
		...
		// prompt user for number shade via command line
		string sNumShades;
		while (sNumShades.empty()) {
			cout << "Input number of toon shades: ";
			cin >> sNumShades;
		}

		numShades = atoi(sNumShades.c_str());
		cout << "Number of toon shades set to " << numShades << "." << endl << endl;
...
void makeMenu() {
	...
	int shaderSubMenu = glutCreateMenu(shaderMenuCallback);
	glutAddMenuEntry("Old", 1);
	glutAddMenuEntry("Phong", 2);
	glutAddMenuEntry("Toon", 3);
	glutAddMenuEntry("Set Num Shades", 4);
	...
	glutAddSubMenu("Set Shader", shaderSubMenu);
...
int main(int argc, char** argv) {
	mat4 test = Angel::identity();
	cout << test[0][0] << "," << test[1][1] << ". " << test[1][2] << endl;
